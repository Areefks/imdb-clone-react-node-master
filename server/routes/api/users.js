...
const mailerAuth = require("../../config/mailer_config_template");
...
router.get("/isloggedin", async (req, res) => {
  ...
  return res.send({ user: req.session.user, response: "Authenticated" });
});
...
router.post("/forgotpassword", async (req, res) => {
  ...
  if (!user) {
    res
      .status(404)
      .send({ response: "The email does not exist in the database" });
  } else {
  ...
  if (id && recoveryLink) {
    ...
    bcrypt.hash(newPassword, saltRounds, async (error, hashedPassword) => {
      if (error) {
        return res
          .status(500)
          .send({ response: "Password could not be updated" });
      }
